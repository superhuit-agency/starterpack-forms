...
<div class="wrap form-submissions-list">
	<h1 class="wp-heading-inline"><?php _ex('Forms Submissions', 'SUPT Form Submissions', 'supt'); ?></h1>
	...
	<form id="form-submissions-list-forms-select" action="<?php echo admin_url(); ?>/edit.php" class="tablenav top">
		...
		<div class="alignleft actions">
			<label for="form-submissions-list-forms-select-top" class="screen-reader-text"><?php _ex('Select a Form', 'SUPT Form Submissions', 'supt'); ?></label>
			<select name="form" id="form-submissions-list-forms-select-top">
				<option value="-1"><?php _ex('Select a Form', 'SUPT Form Submissions', 'supt'); ?></option>
				<?php foreach ($args['forms'] as $item) : ?>
					<?php printf( '<option value="%s"%s>%s (%d)</option>', $item['id'], ($item['active'] ? ' selected' : ''), $item['title'], $item['count'] ); ?>
				<?php endforeach; ?>
			</select>
			<input type="submit" class="button action" value="<?php _ex('Show Submissions', 'SUPT Form Submissions', 'supt'); ?>">
		</div>
	</form>
	<?php if ( isset($args['form']) ) : ?>
		...
		<form action="<?php echo admin_url(); ?>/edit.php" class="tablenav top">
			...
			<div class="alignleft actions bulkactions">
				<?php if ( $args['user_can_export'] ) : ?>
					<input type="submit" name="export-results" class="button button-primary button-export" value="<?php _ex('Export All', 'Export all results', 'supt'); ?>"/>
				<?php endif; ?>
				<?php if ( $args['user_can_delete'] ) : ?>
					<input type="submit" name="delete-results" class="button button-delete" value="<?php _ex('Delete All', 'Delete all results', 'supt'); ?>"/>
				<?php endif; ?>
			</div>
			<?php if ( isset($args['months']) && count($args['months']) > 1 ) : ?>
				<div class="alignleft actions">
					<select name="form-submissions-list-from" id="form-submissions-list-forms-filter-date_from">
						<option value=""><?php _ex('From: ', 'SUPT Form Submissions', 'supt'); ?></option>
						<?php foreach ($args['months'] as $ts) : ?>
							<?php printf( '<option value="%s"%s>%s</option>', $ts, ($args['results_from'] === $ts ? ' selected' : ''), date('M Y', $ts ) ); ?>
						<?php endforeach; ?>
					</select>
					<label class="screen-reader-text" for="form-submissions-list-forms-filter-date_from"><?php _ex('From', 'SUPT Form Submissions', 'supt'); ?></label>
					<select name="form-submissions-list-until" id="form-submissions-list-forms-filter-date_until">
						<option value=""><?php _ex("Until", 'SUPT Form Submissions', 'supt'); ?></option>
					...
					<label class="screen-reader-text" for="form-submissions-list-forms-filter-date_until"><?php _ex("Until", 'SUPT Form Submissions', 'supt'); ?></label>
					<input type="submit" name="filter_action" id="post-query-submit" class="button" value="<?php _ex('Filter', 'SUPT Form Submissions', 'supt'); ?>">
				</div>
			<?php endif; ?>
			...
			<div class="tablenav-pages one-page">
				...
				<?php if ( $nb_results ) : ?>
					<span><?php printf( _n('1 submission', '%d submissions', $nb_results, 'SUPT Form Submissions', 'supt'), $nb_results ); ?></span>
				<?php endif; ?>
			</div>
			<br class="clear">
		</form>
		...
		<div id="form-submissions-list-form_<?php echo $args['form']['id']; ?>" class="table-wrapper" method="post" data-form-sent>
			<h3 class="screen-reader-text"><?php _ex('Submissions', 'SUPT Form Submissions', 'supt'); ?></h3>
			<table class="wp-list-table widefat fixed striped posts">
				<thead>
					<tr>
						<th scope="col" id="date" class="manage-column column-date"><?php _ex('Date', 'SUPT Form Submissions', 'supt'); ?></th>
				...
				<tbody id="the-list" class="ui-sortable">
					<?php foreach ( $args['form']['results'] as $itemId => $item) : ?>
						<tr id="sent-<?php echo $itemId; ?>" class="hentry">
							<td class="date column-date has-row-actions column-primary" data-colname="<?php _ex('Date', 'SUPT Form Submissions', 'supt'); ?>">
									...
									<div class="row-actions">
										<span class="trash">
											<?php printf(
												...
												_x('Delete permanently', 'Delete result item', 'supt'),
												_x('Delete permanently', 'Delete result item', 'supt')
											); ?>
										</span>
									</div>
